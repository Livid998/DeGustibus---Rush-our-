...
func _test_builder_and_seating(world: RestaurantWorld) -> void:
	...
	for chair: PlacedObject in attached_chairs:
		chairs_face_table = chairs_face_table and chair.rotation_steps == world.seat_rotation_for_slot(chair.attachment_slot, table.rotation_steps) and chair.position.distance_to(table.position) <= 1.94
		...
		separate_seat_anchors = separate_seat_anchors and absf(Vector3(assignment.chair_position).distance_to(table.position) - 1.89) < 0.02 and absf(Vector3(assignment.position).distance_to(table.position) - 1.45) < 0.02
	...
	for chair: PlacedObject in moved_chairs:
		chairs_followed = chairs_followed and chair.support_uid == table.uid and chair.position.distance_to(table.position) <= 1.94
